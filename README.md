...
- 首页直接发布文字/图片/视频/来源、独立发布页回退、预审核状态、详情、平铺评论、点赞和关注
- 每帖最多四张安全图片：真实解码、EXIF 方向校正、元数据清除、像素限制并统一重编码为 WebP
- 每帖一个不超过 20 MB、5 分钟的安全视频：严格编码检查、FFmpeg 重封装、元数据清除、WebP 封面和 Range 播放
- X / YouTube 来源分享：本地规范化 ID、固定官方 oEmbed、可追溯署名卡片和 YouTube 隐私增强播放器；不下载第三方媒体
- 成员公开主页、“我的社区”、安全头像上传/替换/删除、资料与密码管理、作者撤回
- 带审核结论与理由的通知中心、绑定真实对象的举报入口
- 待审内容/评论专用队列、运营总览和“运营/审核”最小权限组
- 桌面与手机响应式布局、CSP、安全缓存边界和双维度限流

头像、图片、视频和视频封面保存在同一个受备份的 `/data` 数据边界中，但不直接公开映射媒体目录。每次读取都会核对规范路径、数据库元数据和成员/内容状态；头像和内容图片都会完整解码、移除来源元数据并统一重编码为 WebP。头像替换采用不可变版本，旧版本至少保留到备份清理窗口。
...
- [X / YouTube 来源分享边界](docs/EXTERNAL_SHARE_ROADMAP.md)
